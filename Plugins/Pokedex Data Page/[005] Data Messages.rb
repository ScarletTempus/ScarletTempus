...
class PokemonPokedexInfo_Scene
  ...
  def pbDataTextFamily(path, species, overlay)
    ...
    mega   = species.mega_stone || species.mega_move
    primal = species.form_name == _INTL("Primal {1}", species.name)
    ultra  = PluginManager.installed?("ZUD Mechanics") && species.ultra_item
    gmax   = PluginManager.installed?("ZUD Mechanics") && @gmax
    if mega || primal || ultra || gmax
      if gmax      then check_form = @form
      elsif ultra  then check_form = species.ultra_base_form
      elsif primal then check_form = 0
      else              check_form = species.unmega_form
      end
