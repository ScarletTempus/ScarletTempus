...
class ItemCraft_Scene
  ...
  def pbStartScene
    ...
    @sprites["itemtext"].baseColor = BASELIGHT
    @sprites["itemtext"].shadowColor = SHADOWLIGHT
    ...
    @sprites["bottombar"].bitmap.fill_rect(0,Graphics.height-32,Graphics.width,32,Color.new(157,171,178))
  ...
  def refreshNumbers(index,volume)
    ...
    ingredients = GameData::Recipe.get(@stock[index]).ingredients
    textpos = []
    textpos.push([_INTL("x{1}",volume),@w+26,@h+106,0,BASELIGHT,SHADOWLIGHT])
    ingredients.length.times do |i|
      ingredient = ingredients[i][0]
      ...
      @yPos[i%3] + 38,
      0,
      (bag_qty >= volume*quantity) ? BASELIGHT : Color.new(248,192,0),
      (bag_qty >= volume*quantity) ? SHADOWLIGHT : Color.new(144,104,0)])
    end
  ...
  def pbRedrawItem(index,volume)
    ...
    textpos.push([sprintf("%s x%d", item.name,recipe_data.yield),@w+98,@h+12,0,BASELIGHT,SHADOWLIGHT])
    ingredients.length.times do |i|
      ingredient = ingredients[i][0]
      quantity = ingredients[i][1]
      if ingredient.is_a?(Symbol)
...
def pbItemCrafter(stock,flags=nil,speech1=nil,speech2=nil)
  ...
  if flags == "Cooking"
    if pbConfirmMessage(_INTL("Would you like to cook something?"))
  ...
  pbMessage(speech2 ? speech2 : _INTL("That should do!"))
end
